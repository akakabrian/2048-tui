...
Plays an MP3 from `twenty48_tui/assets/music/` via `paplay` (PulseAudio /
...
_SIGNATURE = "twenty48_tui/assets/music/"


def _cleanup_orphans() -> None:
    """Kill any prior 2048 music bash loops + their paplay/afplay
...
TRACKS: list[Path] = [
    MUSIC_DIR / "happy_song.mp3",
    MUSIC_DIR / "km_easy_lemon.mp3",
    MUSIC_DIR / "km_fluffing_a_duck.mp3",
]

ATTRIBUTIONS = [
    "Happy — Alex McCulloch, CC0 1.0",
    "Easy Lemon — Kevin MacLeod (incompetech.com), CC-BY 4.0",
...
class MusicPlayer:
    def __init__(self, enabled: bool = True, tracks: list[Path] | None = None) -> None:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            loop_cmd = (
                "trap 'kill -TERM $(jobs -p) 2>/dev/null; exit 0' TERM INT HUP; "
